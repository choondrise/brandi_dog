...
def train_chunk(
    ...
    for game_index in range(games):
        opponent = sample_opponent(opponent_pool, rng)
...
def _run_training_game(
    ...
    state = engine.reset()
    opponent_b1 = build_agent_from_spec(opponent, seed=seed * 17 + 1, device=device, encoder=encoder)
    opponent_b2 = build_agent_from_spec(opponent, seed=seed * 17 + 2, device=device, encoder=encoder)
    agents = {
        PlayerId.A1: policy,
        PlayerId.A2: policy,
    ...
    winner = state.winner
    reward = 100.0 if winner == Team.A else -100.0
    if winner is None:
        score_a, score_b = safe_zone_score(state)
        reward = 10.0 * (score_a - score_b)
    ...
    score_a, score_b = safe_zone_score(state)
    return GameOutcome(winner=winner, turns=turns, capped=capped, score_a=score_a, score_b=score_b, reward=reward, update=update)
...
def make_policy(checkpoint_path: str, output_dir: Path, args) -> TrainableRankingPolicy:
    config = RLTrainingConfig(
        ...
        rotate_trained_agent_positions=False,
        device=args.device,
...
def main() -> None:
    ...
    for iteration in range(1, args.iterations + 1):
        ...
        heuristic_eval = evaluate_agent(
            str(candidate_path), OpponentSpec("heuristic", "heuristic", 1.0), args.eval_games, args.seed + iteration * 10_000 + 1, args.max_turns, args.device, args.encoder, args.eval_workers
        )
        ranking_eval = evaluate_agent(
            str(candidate_path), OpponentSpec("ranking_model_v2", "deep_learning", 1.0, ranking_checkpoint), args.eval_games, args.seed + iteration * 10_000 + 2, args.max_turns, args.device, args.encoder, args.eval_workers
        )
        champion_eval = evaluate_agent(
            str(candidate_path), OpponentSpec("old_champion", "deep_learning", 1.0, champion_checkpoint), args.eval_games, args.seed + iteration * 10_000 + 3, args.max_turns, args.device, args.encoder, args.eval_workers
        )
        mc_eval = None
        if args.use_mc_eval:
            mc_eval = evaluate_agent(
                str(candidate_path), OpponentSpec("monte_carlo", "monte_carlo", 1.0), args.mc_eval_games, args.seed + iteration * 10_000 + 4, args.max_turns, args.device, args.encoder, args.eval_workers
            )
        ...
        print(
            f"Evaluation iter {iteration}: heuristic={heuristic_eval.winrate:.3f}, "
            f"ranking_v2={ranking_eval.winrate:.3f}, old_champion={champion_eval.winrate:.3f}, "
            f"mc={None if mc_eval is None else round(mc_eval.winrate, 3)}, promoted={promoted}: {reason}",
            flush=True,
        )
